...
export default function Today() {
    ...
    let { data , loading } = useFetch(`/forecast.json?aqi=yes&days=1`)
    ...
    <Container maxW={'3xl'} className="today-component container">
        <Card p={'0'}>
            ...
            borderTopRightRadius={'6px'}>
                <Heading size={['md']} fontWeight={'500'} color={'white'}> {location} </Heading>
                <Heading size={'sm'} color={'white'} fontWeight={'regular'} >  As of {time} </Heading>
            </Flex>

            <Box className="card-body" p={'0px 15px'}>
                <Flex justifyContent={'space-between'} alignItems={'center'} mt={'10px'}>
                    <Heading size={'4xl'} >{data?.current?.temp_c}°</Heading>
                    <img src={data?.current?.condition?.icon} alt="" />
                </Flex>
                <Heading size={'md'} mt={'10px'} mb={'6px'} > { data?.current?.condition?.text } </Heading>
                <HStack gap={'10px'} alignItems={'center'} pb={'10px'}>
                    <Heading size={'md'} >Day {data?.forecast?.forecastday[0]?.day?.maxtemp_c} °</Heading>
                    <Heading size={'md'} fontWeight={'900'} mb={'8px'} >.</Heading>
                    <Heading size={'md'} >Night {data?.forecast?.forecastday[0]?.day?.mintemp_c} °</Heading>
                </HStack>
            </Box>
        ...
        <Card mt={'15px'}>
            ...
            <Grid templateColumns='repeat(4, 1fr)' gap={2} mt={'15px'}>
                    <GridItem>
                        <VStack  borderRight={'1px solid rgba(0, 0, 0, 0.36)'}>
                            <Text fontWeight={'400'}>Morning</Text>
                            <Heading color={'blue.600'}> {todayForecast?.hour[6]?.temp_c}°</Heading>
                    ...
                    <GridItem>
                        <VStack  borderRight={'1px solid rgba(0, 0, 0, 0.36)'}>
                            ...
                            <Heading color={'blue.600'}> {todayForecast?.hour[13]?.temp_c}°</Heading>
                    ...
                    <GridItem>                        
                        <VStack  borderRight={'1px solid rgba(0, 0, 0, 0.36)'}>
                            ...
                            <Heading color={'blue.600'}> {todayForecast?.hour[20]?.temp_c}°</Heading>
                    ...
                    <GridItem>                        
                        <VStack >
                            ...
                            <Heading color={'blue.600'}> {todayForecast?.hour[0]?.temp_c}°</Heading>
        ...
        <Card mt={'15px' } mb={'60px'}>
            ...
            <Grid templateColumns={'1fr 1fr'} gap={'12px'} mt={'30px'}  mx={'-20px'} rowGap={'10px'} >
                ...
                <Flex justifyContent={'space-between'} alignItems={'center'} pb={'8px'}  px={'20px'}>
                    <HStack>
